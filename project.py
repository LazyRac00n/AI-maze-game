...
class Maze(tk.Tk, object): # Forked from Tkinter
    def __init__(self, unit_pixels = 40, grid_height = 5, grid_width = 5):
    ...
    def __build_maze(self):
        ...
        # Hell 1 
        hell1_center = origin + np.array([self.UNIT * 3, self.UNIT]) # center of (2, 1)
        x0, y0 = hell1_center[0] - 15, hell1_center[1] - 15
        x1, y1 = hell1_center[0] + 15, hell1_center[1] + 15
        self.hell1 = self.canvas.create_oval(x0, y0, x1, y1, fill = 'red') # Create rectangle with coordinates x1,y1,x2,y2
        ...
        self.hell2 = self.canvas.create_oval(x0, y0, x1, y1, fill = 'red')
        ...
        self.endpoint = self.canvas.create_oval(x0, y0, x1, y1, fill = 'blue') # Create oval with coordinates x1,y1,x2,y2

        # Explorer 
        x0, y0 = origin[0] - 15, origin[1] - 15
        x1, y1 = origin[0] + 15, origin[1] + 15
        self.explorer = self.canvas.create_oval(x0, y0, x1, y1, fill = 'black')
    ...
    def move(self, action):
        ...
        # reward function
        if next_coords in [self.canvas.coords(self.endpoint)]:
            reward = 1
            done = True
        ...
        # return observation
        observation_ = (np.array(next_coords[:2]) - np.array(self.canvas.coords(self.endpoint)[:2])) / (self.MAZE_H * self.UNIT)
    ...
    def reset(self):
        ...
        # return observation
        return (np.asarray(self.canvas.coords(self.explorer)[:2]) - np.asarray(self.canvas.coords(self.endpoint)[:2])) / (self.MAZE_H * self.UNIT)
